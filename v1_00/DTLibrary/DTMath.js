import { sineArray, arctanArray } from "./DTMathHelper.js";
...
let sineScaled = function (degreesScaled) {
    ...
    if (newDegreesTimes32 < 180 * 32)
        return sineArray[newDegreesTimes32];
    else {
        let translatedDegrees = newDegreesTimes32 - 180 * 32;
        return -sineArray[translatedDegrees];
    }
};
...
let arcTangentScaled = function (x, y) {
    ...
    if (x > 0 && y > 0) {
        ...
        if (result >= arctanArray.length)
            result = arctanArray.length - 1;
        let angle = arctanArray[result];
        return angle;
    }
    if (x < 0 && y > 0) {
        let negativeX;
...
export { normalizeDegreesScaled, sineScaled, cosineScaled, arcTangentScaled, arcTangentScaledSafe };
